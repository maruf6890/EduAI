...
export default function CardFlip({ classroom }: CardFlipProps) {
  ...
  return (
    <div
      className={cn(
        'flex flex-col overflow-hidden rounded-sm',
        'bg-main border-2 border-border-main',
        'transition-shadow duration-300 hover:shadow-[0_8px_40px_rgba(139,92,246,0.1)]',
      )}
    >
      {/* Header strip */}
      <div className="relative h-[100px] w-full bg-gradient-to-br from-brand-primary/50 to-brand-primary">
        {classroom.coverImage && (
          <img
            src={classroom.coverImage}
            alt={classroom.name}
        ...
        {classroom.semester && (
          <span className="absolute bottom-3 left-4 text-[11px] font-medium text-text-main/60">
            Semester: {classroom.semester}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-3 px-5 py-4">
        ...
        {classroom.description && (
          <p className="line-clamp-2 text-[12px] leading-relaxed text-text-main/70">
            {classroom.description}
          </p>
        )}
        ...
        <button
          onClick={() => router.push(`/dashboard/classrooms/${classroom.id}`)}
          className={cn(
            'mt-1 w-full rounded-sm py-2.5 px-4',
            'flex items-center justify-center gap-2',
